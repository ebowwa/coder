...
export interface UseTokenReturn {
  ...
  // Mint
  mint: (to: string, amount: string) => Promise<string | null>;
  mintAsync: (to: string, amount: string) => Promise<string>;
  isMinting: boolean;
...
export function useToken(options: UseTokenOptions = {}): UseTokenReturn {
  ...
  // Mint wrapper
  const mint = useCallback(async (to: string, amount: string): Promise<string | null> => {
    if (!isContractDeployed || decimals === undefined) {
      throw new Error('Token contract not available');
    }

    const amountWei = parseUnits(amount, decimals);
    return mintWrite(to, amountWei);
  }, [isContractDeployed, decimals, mintWrite]);

  const mintAsync = useCallback(async (to: string, amount: string): Promise<string> => {
    if (!isContractDeployed || decimals === undefined) {
      throw new Error('Token contract not available');
    }

    const amountWei = parseUnits(amount, decimals);
    return mintWriteAsync(to, amountWei);
  }, [isContractDeployed, decimals, mintWriteAsync]);
