...
class ActionSignerHandle {
  ...
  async sign(
    ...
    const id = `action-${this.sessionId}-${this.sequence}`;
    ...
    const action: SignedAction = {
      ...
      paramsHash,
      timestamp,
      intentId: this.intentId || undefined,
      signature: "",
  ...
  async verify(action: SignedAction): Promise<ActionVerification> {
    ...
    return {
      ...
      chainIntact: action.prevHash === this.lastHash || action.sequence === 1,
      hashValid: true,
      intentVerified: !this.intentId || action.intentId === this.intentId,
      valid,
...
class DriftDetectorHandle {
  ...
  record(action: SignedAction): void {
    ...
    if (this.actionTimes.length > 1) {
      const timeSpan = (this.actionTimes[this.actionTimes.length - 1] - this.actionTimes[0]) / 1000;
      if (timeSpan > 0) {
  ...
  detect(): DriftResult {
    ...
    const driftScore = indicators.length > 0
      ? indicators.reduce((sum, i) => sum + i.severity, 0) / indicators.length
      : 0;

    return {
      driftScore,
      driftDetected: driftScore > this.threshold,
      indicators,
      recommendation: driftScore > 0.7
        ? "Block actions and require user intervention"
        : driftScore > 0.5
        ? "Require approval for all actions"
        : driftScore > this.threshold
        ? "Increase monitoring"
        : "Continue normal operation",
      shouldBlock: driftScore > 0.7,
    };
  }
