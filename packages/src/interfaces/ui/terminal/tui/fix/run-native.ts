...
export class NativeTUI {
  ...
  private handleInput(data: string): void {
    ...
    if (data === "\x17") {
      const result = this.inputManager.deleteWordBefore(
        this.state.inputValue,
        this.state.cursorPos
      );
      this.state.inputValue = result.text;
      this.state.cursorPos = result.pos;
      this.render();
      return;
    }

    // Alt+Backspace (delete word before cursor - alternative binding
    if (data === "\x1b\x7f") {
      const result = this.inputManager.deleteWordBefore(
        this.state.inputValue,
        this.state.cursorPos
    ...
    if (data === "\x1b\x1b\x44") {
      this.state.cursorPos = this.inputManager.moveWordLeft(
        this.state.inputValue,
        this.state.cursorPos
      );
      this.render();
    ...
    if (data === "\x1b\x1b\x43") {
      this.state.cursorPos = this.inputManager.moveWordRight(
        this.state.inputValue,
        this.state.cursorPos
      );
      this.render()
    ...
    // Alt+B / Option+B (word left - alternative binding)
    if (data === "\x1bb") {
      this.state.cursorPos = this.inputManager.moveWordLeft(
        this.state.inputValue,
        this.state.cursorPos
    ...
    // Alt+F / Option+F (word right - alternative binding
    if (data === "\x1bf") {
      this.state.cursorPos = this.inputManager.moveWordRight(
        this.state.inputValue,
        this.state.cursorPos
